...
import datetime
links = []
List= ["``Please Enter Fighter 1``","``Please Enter Fighter 2``","``Grabbing links...``"]


def get_links(Link):
	get_page = r.get(f"https://www.boxingstreams100.com/{Link}")
	...
	discord = soup.find("a",href=re.compile('https://discord.gg/CtUAFZg'))
	#print(n)
...
class nfl(commands.Cog):
	...
	async def boxing(self,ctx):
		...
		link_part_1 = (f"{team1}-vs-{team2}-live-stream/")
		link_part_2 = (f"{team2}-vs-{team1}-live-stream/")

		#await ctx.send(link_part_1)
		#await ctx.send(link_part_2)
		url_1 = r.get(f"https://www.boxingstreams100.com/{link_part_1}")
		url_2 = r.get(f"https://www.boxingstreams100.com/{link_part_2}")
		#await ctx.send(url_1)
		#await ctx.send(url_2)
		print(url_1.request.url)
		print(url_2.request.url)
		if url_1.request.url != "https://www.boxingstreams100.com":
			get_links(link_part_1)
		if url_2.request.url != "https://www.boxingstreams100.com":
	...
	#	message = ('\n'.join(map(str, links)))
		...
		if links != []:
			with ctx.typing():
				...
				embed = discord.Embed(title = f"{team1} v {team2} :boxing_glove: ", color=random.randint(0, 0xFFFFFF))
		    ...
		    await ctx.send("``Failed to fetch links`` - *Summon The Command* ``!ufcnames`` *To Get a List Of All Matches Available and The Correct Team Names*\n\nIf There Is a Bug, report this for support using ``!report bugreason``")
		    await channel.purge(limit=7, check = delete, bulk = True)
