...
def get_names():
	get_page = r.get(f"https://soccerstreams-100.com/")
	...
	get_page2 = r.get(f"https://soccerstreams-100.com/page/2/")
...
class send_names(commands.Cog):
	...
	async def names(self,ctx):
		...
		em.set_footer(text= f"Requested By: {ctx.author.name}#{ctx.author.discriminator} • !names", icon_url=f"{ctx.author.avatar_url}")
