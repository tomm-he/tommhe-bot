...
class nfl(commands.Cog):
	...
	async def ufc(self,ctx):
		...
		url_1 = r.get(f"https://www.mmastreams100.com/{link_part_1}")
		url_2 = r.get(f"https://www.mmastreams100.com/{link_part_2}")
		#await ctx.send(url_1)
		#await ctx.send(url_2)
		print(url_1.request.url)
		print(url_2.request.url)
		if url_1.request.url != "https://www.mmastreams100.com":
			get_links(link_part_1)
		if url_2.request.url != "https://www.mmastreams100.com":
			get_links(link_part_2)
		i = 0
		digit = random.randint(6,20)
	#	message = ('\n'.join(map(str, links)))
		select = []
		if links != []:
			with ctx.typing():
				...
				embed = discord.Embed(title = f"{team1} v {team2} :martial_arts_uniform:", color=random.randint(0, 0xFFFFFF))
		    ...
		    await ctx.send("``Failed to fetch links`` - *Summon The Command* ``!ufcnames`` *To Get a List Of All Matches Available and The Correct Team Names* And/Or  Summon ``!ufchelp`` For Help")
		    await channel.purge(limit=7, check = delete, bulk = True)
