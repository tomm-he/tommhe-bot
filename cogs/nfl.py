...
def get_links(Link):
	get_page = r.get(f"https://www.nflstreams100.com/{Link}")
...
class nfl(commands.Cog):
	...
	async def nfl(self,ctx):
		...
		url_1 = r.get(f"https://www.nflstreams100.com/{link_part_1}")
		url_2 = r.get(f"https://www.nflstreams100.com/{link_part_2}")
		#await ctx.send(url_1)
		#await ctx.send(url_2)
		print(url_1.request.url)
		print(url_2.request.url)
		if url_1.request.url != "https://www.nflstreams100.com":
			get_links(link_part_1)
		if url_2.request.url != "https://www.nflstreams100.com":
			get_links(link_part_2)
		i = 0
		digit = random.randint(7,20)
	#	message = ('\n'.join(map(str, links)))
		select = []
		if links != []:
			with ctx.typing():
				...
				embed = discord.Embed(title = f"{team1} v {team2} :football: ", color=random.randint(0, 0xFFFFFF))
		    ...
		    await ctx.send("``Failed to fetch links`` - *Summon The Command* ``!nflnames`` *To Get a List Of All Matches Available and The Correct Team Names* And/Or  Summon ``!nflhelp`` For Help\n\nIf There Is a Bug, report this for support using ``!report bugreason``")
		    await channel.purge(limit=7, check = delete, bulk = True)
