...
def get_links(Link):
	get_page = r.get(f"https://www.nbastreams100.com/{Link}")
	...
	discord = soup.find("a",href=re.compile('https://discord\.gg/zCrbcqy'))
	#print(n)
...
class nfl(commands.Cog):
	...
	async def nba(self,ctx):
		...
		url_1 = r.get(f"https://www.nbastreams100.com/{link_part_1}")
		url_2 = r.get(f"https://www.nbastreams100.com/{link_part_2}")
		#await ctx.send(url_1)
		#await ctx.send(url_2)
		print(url_1.request.url)
		print(url_2.request.url)
		if url_1.request.url != "https://www.nbastreams100.com":
			get_links(link_part_1)
		if url_2.request.url != "https://www.nbastreams100.com":
	...
	#	message = ('\n'.join(map(str, links)))
		...
		if links != []:
			with ctx.typing():
				...
				embed = discord.Embed(title = f"{team1} v {team2} :basketball: ", color=random.randint(0, 0xFFFFFF))
		    ...
		    await ctx.reply("``Failed to fetch links`` - *Summon The Command* ``!nbanames`` *To Get a List Of All Matches Available and The Correct Team Names* And/Or  Summon ``!nbahelp`` For Help. Sometimes it is the second meeting of the two teams. in that case add a 2 at the end of the second team. ``Example: los angeles lakers 2``\n\n If There Is a Bug, report this for support using ``!report bugreason``")
		    await channel.purge(limit=6, check = delete, bulk = True)
