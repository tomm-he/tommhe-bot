...
import datetime
links = []
List= ["``Please Enter The Motogp Name And GP`","``Grabbing links...``"]

def get_links(Link):
	get_page = r.get(f"https://www.motogpstreams100.com/{Link}")
	...
	discord = soup.find("a",href=re.compile('https://discord.gg/ef7YZKb'))
	#print(n)
...
class nfl(commands.Cog):
	...
	async def motogp(self,ctx):
		...
		await ctx.send("``Please Enter The Motogp Name And GP``")
		...
		url_1 = r.get(f"https://www.motogpstreams100.com/{link_part_1}")
		#await ctx.send(url_1)
		#await ctx.send(url_2)
		print(url_1.request.url)
		if url_1.request.url != "https://www.motogpstreams100.com":
	...
	#	message = ('\n'.join(map(str, links)))
		...
		if links != []:
			with ctx.typing():
				...
				embed = discord.Embed(title = f"{team1} :motorcycle:", color=random.randint(0, 0xFFFFFF))
		    ...
		    await ctx.send("``Failed to fetch links`` - *Summon The Command* ``!motogpnames`` *To Get a List Of All Matches Available and The Correct Team Names* And/Or  Summon ``!motogphelp`` For Help")
